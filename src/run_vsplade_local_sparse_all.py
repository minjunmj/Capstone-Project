"""Dense (BiModernVBERT) vs. Local Sparse (V-SPLADE, per-tile) vs. RSF Fusion
across all ViDoRe v1 subsets. Loads both models once, then iterates over
subsets (mirrors src/run_vsplade_dense_sparse_all.py's pattern). See
src/run_vsplade_local_sparse.py for the single-subset version.
"""
...
def main():
    ...
    for subset in VIDORE_V1_BEIR_SUBSETS:
        t0 = time.time()
        run_dir = make_run_dir("outputs", f"vsplade_local_sparse_{subset}")
        save_config_snapshot(run_dir, {
            "dense_model": "ModernVBERT/bimodernvbert", "sparse_model": "naver/v-splade-quality",
            "sparse_granularity": "local (per Idefics3 tile)",
        ...
        try:
            ...
            logger.info(f"tiles/doc mean={sum(tile_counts)/len(tile_counts):.1f} "
                        f"min={min(tile_counts)} max={max(tile_counts)}")
            logger.info(f"dense-only={metrics_dense}")
            logger.info(f"sparse-only(local)={metrics_sparse}")
            ...
            save_json(run_dir / "metrics.json", {
                ...
                "mean_tiles_per_doc": sum(tile_counts) / len(tile_counts),
    ...
    out_path = Path("outputs/tables/vsplade_local_sparse_all_v1.csv")
